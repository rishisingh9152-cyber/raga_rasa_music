...
async def detect_emotion(request: EmotionDetectRequest):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[Emotion] Rasa prediction failed: {rasa_err}")
            rasa_result = {"rasa": "Shaant", "confidence": 0.8}
        ...
        if db is not None:
            ...
            logger.warning("[Emotion] DB unavailable; skipping session emotion update")
